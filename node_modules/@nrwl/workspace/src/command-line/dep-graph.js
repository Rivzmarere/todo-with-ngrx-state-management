...
function projectsToHtml(projects, graph, affected, focus, groupByFolder, exclude) {
    let f = fs_1.readFileSync(path_1.join(__dirname, '../core/dep-graph/dep-graph.html')).toString();
    f = f
        ...
        .replace(`window.groupByFolder = null`, `window.groupByFolder = ${!!groupByFolder}`)
        .replace(`window.exclude = null`, `window.exclude = ${JSON.stringify(exclude)}`);
    if (focus) {
...
function generateGraph(args, affectedProjects) {
    ...
    if (args.file) {
        ...
        if (ext === 'html') {
            ...
            html = html.replace(/<(script.*|link.*)="(.*\.(?:js|css))"(><\/script>| \/>?)/g, '<$1="static/$2"$3');
...
function startServer(html, host) {
